...
export function ActivitiesList({ activities, currentUserId, signedIn }: { activities: Activity[]; currentUserId?: string | null; signedIn: boolean }) {
      ...
      {filtered.length ? (
        <div className="activity-grid">
          {filtered.map((activity) => (
            <ActivityCard key={activity.id} activity={activity} currentUserId={currentUserId} signedIn={signedIn} />
          ))}
        </div>
      ) : (
